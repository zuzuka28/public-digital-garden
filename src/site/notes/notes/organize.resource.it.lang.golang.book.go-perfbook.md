...
1. Advanced tips for writing _really_ fast software
...
Think of this as optimizing _your_ time.
...
verify that your change really _is_ lowering CPU. Sometimes a change you
...
issues will be too risky near the deadline. Note that _during_ development,
...
performance every 18 _years_, a stark contrast with the (slightly
misunderstood interpretation) of Moore's Law that doubles processor
performance every 18 _months_. Algorithmic improvements work at larger magnitudes.
...
programmer's job is to use the transformations compilers _can't_ do.
...
If the domain is small enough, the _entire_ set of results could be
...
select a probably not _bad_ thing. For example, a treap is a
...
TODO: - Algorithm Design Manual: http://algorist.com/algorist.html - How To Solve It By Computer - to what extent is this a "how to write algorithms" book? If you're going to change
...
effective in many cases. Similarly, randomized cache _insertion_ can limit your
...
Every optimization codifies an assumption about your data. These _must_ be
...
_and_ that they all fall in a single day, we can write custom time parsing
...
add code complexity in exchange for speed. And you can _always_ make code
